...
function BarChartSVG({ data }: { data: { month: string; count: number }[] }) {
  ...
  if (!data.length) return <div className="h-44 flex items-center justify-center text-gray-300 text-xs">Chưa có dữ liệu</div>;
...
function DonutChart({ occupied, total }: { occupied: number; total: number }) {
  ...
  return (
    <svg viewBox="0 0 120 120" className="w-28 h-28 shrink-0">
      ...
      <text x={cx} y={cy + 10} textAnchor="middle" fontSize={8} fill="#94a3b8">công suất</text>
    </svg>
  );
}
...
const APPT_STATUS: Record<string, { label: string; cls: string }> = {
  ...
  IN_PROGRESS:{ label: 'Đang khám',    cls: 'bg-blue-100 text-blue-700' },
  CONFIRMED:  { label: 'Đã xác nhận',  cls: 'bg-indigo-100 text-indigo-700' },
  CHECKED_IN: { label: 'Đã check-in',  cls: 'bg-purple-100 text-purple-700' },
  PENDING:    { label: 'Chờ',          cls: 'bg-yellow-100 text-yellow-700' },
  CANCELLED:  { label: 'Hủy',          cls: 'bg-red-100 text-red-700' },
  NO_SHOW:    { label: 'Vắng mặt',     cls: 'bg-gray-100 text-gray-500' },
};
const LAB_PRIORITY: Record<string, { label: string; cls: string }> = {
  URGENT:    { label: 'Khẩn cấp',    cls: 'bg-red-100 text-red-700' },
  PRIORITY:  { label: 'Ưu tiên',     cls: 'bg-yellow-100 text-yellow-700' },
  NORMAL:    { label: 'Bình thường', cls: 'bg-green-100 text-green-700' },
  PROCESSING:{ label: 'Đang xử lý', cls: 'bg-gray-100 text-gray-500' },
};

// ─── Quick Actions ────────────────────────────────────────────────────────────
const QUICK_ACTIONS = [
  { label: 'Thêm bệnh nhân', icon: Users,        color: 'bg-blue-50 text-blue-600 hover:bg-blue-100',        href: '/patients' },
  { label: 'Đặt lịch khám',  icon: CalendarDays, color: 'bg-emerald-50 text-emerald-600 hover:bg-emerald-100', href: '/appointments' },
  { label: 'Tạo phiếu XN',   icon: FlaskConical, color: 'bg-orange-50 text-orange-600 hover:bg-orange-100',   href: '/lab' },
  { label: 'Lên lịch PT',    icon: Scissors,     color: 'bg-violet-50 text-violet-600 hover:bg-violet-100',   href: '/surgery' },
  { label: 'Đợt điều trị',   icon: Stethoscope,  color: 'bg-teal-50 text-teal-600 hover:bg-teal-100',         href: '/encounters' },
  { label: 'Thanh toán',     icon: DollarSign,   color: 'bg-amber-50 text-amber-600 hover:bg-amber-100',      href: '/billing' },
];

const RANGE_LABELS: Record<Range, string> = { today: 'Hôm nay', week: 'Tuần này', month: 'Tháng này' };
...
export default function DashboardPage() {
  ...
  const STAT_CARDS = [
    { title: 'Tổng bệnh nhân',    value: s.totalPatients ?? 0,                                                    icon: Users,        color: 'blue',    trend: { pct: 8.2,  label: 'vs hôm qua' } },
    { title: 'Ca khám hôm nay',   value: s.todayAppointments ?? 0,                                                icon: CalendarDays, color: 'emerald', trend: { pct: 5.1,  label: 'vs hôm qua' } },
    { title: 'Doanh thu',         value: `${((s.todayRevenue ?? 0) / 1_000_000).toFixed(1)}M đ`,                  icon: DollarSign,   color: 'amber',   trend: { pct: 12.4, label: 'vs hôm qua' } },
    { title: 'Bác sĩ hoạt động',  value: s.activeDoctors ?? 0,                                                    icon: UserRound,    color: 'violet',  trend: { pct: 0,    label: 'không đổi' } },
    { title: 'Chờ xác nhận',      value: s.pendingAppointments ?? 0,                                              icon: Clock,        color: 'rose',    trend: { pct: -3.2, label: 'vs hôm qua' } },
    { title: 'Đang điều trị',     value: s.activeEncounters ?? 0,                                                 icon: Activity,     color: 'teal',    trend: { pct: 2.1,  label: 'vs hôm qua' } },
    { title: 'Giường đang dùng',  value: `${s.occupiedBeds ?? 0}/${s.totalBeds ?? 0}`,                            icon: BedDouble,    color: 'indigo',  sub: `${s.bedOccupancyRate ?? 0}% công suất` },
    { title: 'XN chờ kết quả',    value: s.pendingLabOrders ?? 0,                                                 icon: FlaskConical, color: 'orange',  trend: { pct: -1.5, label: 'vs hôm qua' } },
    { title: 'Thuốc sắp hết',     value: s.lowStockCount ?? 0,                                                    icon: Package,      color: 'pink',    sub: 'Dưới mức tối thiểu' },
    { title: 'Phẫu thuật hôm nay',value: s.todaySurgeries ?? 0,                                                   icon: Scissors,     color: 'red',     trend: { pct: 1.0,  label: 'vs hôm qua' } },
  ];
    ...
    <Layout>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">Tổng quan hệ thống — {RANGE_LABELS[range]}</p>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5 mb-5">
        <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-5">
          <div className="flex items-center justify-between mb-4">
            <div><h3 className="font-semibold text-gray-900 text-sm">Lịch khám theo tháng</h3><p className="text-xs text-gray-400 mt-0.5">6 tháng gần nhất</p></div>
            <Link href="/appointments" className="text-xs text-primary-500 hover:underline flex items-center gap-1">Xem thêm <ArrowRight size={12} /></Link>
          </div>
          <BarChartSVG data={s.appointmentChart || []} />
        </div>
        <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-5">
          <div className="flex items-center justify-between mb-4">
            <div><h3 className="font-semibold text-gray-900 text-sm">Doanh thu theo tháng</h3><p className="text-xs text-gray-400 mt-0.5">Xu hướng 6 tháng</p></div>
            <Link href="/billing" className="text-xs text-primary-500 hover:underline flex items-center gap-1">Xem thêm <ArrowRight size={12} /></Link>
          </div>
          <AreaChartSVG data={s.revenueChart || []} />
        </div>
        <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-5">
          <div className="flex items-center justify-between mb-4">
            <div><h3 className="font-semibold text-gray-900 text-sm">Tình trạng giường</h3><p className="text-xs text-gray-400 mt-0.5">Hiện tại</p></div>
            <Link href="/beds" className="text-xs text-primary-500 hover:underline flex items-center gap-1">Bản đồ <ArrowRight size={12} /></Link>
          </div>
          <div className="flex items-center gap-4">
            <DonutChart occupied={s.occupiedBeds ?? 0} total={s.totalBeds ?? 0} />
            <div className="space-y-2.5 flex-1">
              {[
                { label: 'Trống', value: (s.totalBeds ?? 0) - (s.occupiedBeds ?? 0), color: 'bg-emerald-400' },
                { label: 'Có BN', value: s.occupiedBeds ?? 0, color: 'bg-blue-400' },
                { label: 'Tổng',  value: s.totalBeds ?? 0,    color: 'bg-slate-300' },
              ].map(r => (
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-5 mb-5">
        ...
        <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-5">
          <div className="flex items-center justify-between mb-4">
            <div><h3 className="font-semibold text-gray-900 text-sm">Lịch khám hôm nay</h3><p className="text-xs text-gray-400 mt-0.5">{apptList.length} lịch</p></div>
            <Link href="/appointments" className="text-xs text-primary-500 hover:underline flex items-center gap-1">Xem tất cả <ArrowRight size={12} /></Link>
          </div>
          <div className="space-y-2">
            {apptList.length === 0 && <p className="text-xs text-gray-400">Chưa có lịch khám hôm nay.</p>}
            {apptList.map(a => {
        ...
        {/* Xét nghiệm chờ kết quả */}
        <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-5">
          <div className="flex items-center justify-between mb-4">
            <div><h3 className="font-semibold text-gray-900 text-sm">Xét nghiệm chờ kết quả</h3><p className="text-xs text-gray-400 mt-0.5">{labList.length} phiếu</p></div>
            <Link href="/lab" className="text-xs text-primary-500 hover:underline flex items-center gap-1">Xem tất cả <ArrowRight size={12} /></Link>
          </div>
          <div className="space-y-2">
            {labList.length === 0 && <p className="text-xs text-gray-400">Không có xét nghiệm chờ xử lý.</p>}
            {labList.map(l => {
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
        ...
        <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-5">
          <div className="flex items-center justify-between mb-4">
            <div><h3 className="font-semibold text-gray-900 text-sm">Tồn kho sắp hết</h3><p className="text-xs text-gray-400 mt-0.5">{stockList.length} mặt hàng</p></div>
            <Link href="/medicines" className="text-xs text-primary-500 hover:underline flex items-center gap-1">Quản lý <ArrowRight size={12} /></Link>
          </div>
          <div className="space-y-3">
            {stockList.length === 0 && <p className="text-xs text-gray-400">Không có mặt hàng dưới ngưỡng tồn kho.</p>}
            {stockList.map(item => {
        ...
        <div className="bg-white rounded-2xl shadow-card border border-gray-100 p-5">
          <div className="flex items-center justify-between mb-4">
            <div><h3 className="font-semibold text-gray-900 text-sm">Hoạt động bác sĩ</h3><p className="text-xs text-gray-400 mt-0.5">Hôm nay</p></div>
            <Link href="/doctors" className="text-xs text-primary-500 hover:underline flex items-center gap-1">Xem tất cả <ArrowRight size={12} /></Link>
          </div>
          <div className="space-y-3">
            {doctorList.length === 0 && <p className="text-xs text-gray-400">Chưa có dữ liệu hoạt động bác sĩ.</p>}
            {doctorList.map(doc => {
