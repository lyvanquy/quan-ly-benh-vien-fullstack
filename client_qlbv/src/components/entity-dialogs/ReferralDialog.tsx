...
export default function ReferralDialog({ frame, onClose }: Props) {
  ...
  const saveMut = useMutation(
    ...
    { onSuccess: () => { qc.invalidateQueries('referrals'); toast.success(isCreate ? 'Da tao phiếu chuyển viện' : 'Da cập nhật'); isCreate ? onClose() : setEditing(false); } }
  );
    ...
    <div>
      <div className="flex items-start gap-4 mb-6">
        ...
        <div className="flex-1">
          <h2 className="text-xl font-bold text-gray-900">{isCreate ? 'Tạo phiếu chuyen vien' : 'Phieu chuyen vien'}</h2>
      ...
      {editing ? (
        ...
        <div className="space-y-4">
          <div className="bg-gray-50 rounded-xl p-4 space-y-3">
            <Row label="Bệnh nhân" value={ref.patient?.name || ref.patientId} />
            ...
            <Row label="Ngày tao" value={new Date(ref.createdAt).toLocaleDateString('vi-VN')} />
          </div>
          {ref.status === 'PENDING' && (
          ...
          {ref.status === 'ACCEPTED' && (
            <button onClick={() => statusMut.mutate('COMPLETED')} className="w-full py-2 bg-green-50 text-green-700 rounded-lg text-sm font-medium hover:bg-green-100">Hoàn thành chuyen vien</button>
          )}
        </div>
      )}
    </div>
