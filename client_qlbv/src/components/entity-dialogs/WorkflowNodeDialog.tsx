...
export default function WorkflowNodeDialog({ frame, onClose }: Props) {
  ...
  const { data: node, isLoading } = useQuery<WorkflowNodeDetail>(
    ['node-dlg', frame.id],
  ...
  if (!node) return <div className="text-center py-12 text-gray-400">Không tìm thấy node</div>;
    ...
    <div>
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          {!editingConfig ? (
            ...
            <div className="flex gap-2">
              <button onClick={saveConfig} className="flex items-center gap-1 text-xs px-2 py-1 bg-primary text-white rounded hover:bg-primary/90"><Save size={10} /> Lưu</button>
              <button onClick={() => setEditingConfig(false)} className="text-xs text-gray-400 hover:text-gray-600">Hủy</button>
            </div>
          )}
        </div>
        {editingConfig ? (
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={() => setShowNewOpt(true)} className="flex items-center gap-1 text-xs text-primary hover:underline"><Plus size={11} /> Thêm option</button>
        </div>
        ...
        {showNewOpt && (
          <div className="flex gap-2 mb-3 p-3 bg-blue-50 rounded-xl">
            ...
            <button onClick={() => addOptMut.mutate(newOpt)} disabled={addOptMut.isLoading} className="px-3 py-1.5 bg-primary text-white rounded-lg text-xs">Thêm</button>
            <button onClick={() => setShowNewOpt(false)} className="px-3 py-1.5 bg-gray-200 text-gray-600 rounded-lg text-xs">Hủy</button>
          </div>
        )}
        ...
        <div className="space-y-2">
          {node.options?.map((opt) => (
            <div key={opt.id} className="border border-gray-200 rounded-xl overflow-hidden">
              <div className="flex items-center gap-3 px-3 py-2.5 bg-gray-50 cursor-pointer"
                ...
                <span className="text-xs font-mono text-gray-400 bg-gray-200 px-2 py-0.5 rounded">{opt.value}</span>
                <button onClick={(e) => { e.stopPropagation(); if (confirm('Xóa option nay?')) deleteOptMut.mutate(opt.id); }}
              ...
              {expandedOpt === opt.id && (
                <div className="px-4 py-3 border-t border-gray-100">
                  {opt.condition && (
                    <p className="text-xs text-gray-500 mb-2 font-mono bg-yellow-50 px-2 py-1 rounded">if: {opt.condition}</p>
                  )}
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs text-gray-500">Actions ({opt.actions?.length || 0})</span>
                    <select className="text-xs border border-gray-200 rounded px-2 py-1"
                      onChange={e => { if (e.target.value) { addActionMut.mutate({ optId: opt.id, type: e.target.value }); e.target.value = ''; } }}>
                      <option value="">+ Thêm action</option>
                  ...
                  <div className="space-y-1.5">
                    {opt.actions?.map((action) => (
                      <div key={action.id} className="flex items-center gap-2 p-2 bg-gray-50 rounded-lg">
                        <span className="text-xs font-mono text-purple-700 bg-purple-50 px-2 py-0.5 rounded flex-1">{action.type}</span>
                        <button onClick={() => runActionMut.mutate(action.id)}
                          className="flex items-center gap-1 text-xs px-2 py-1 bg-green-50 text-green-700 rounded hover:bg-green-100">
                          <Play size={10} /> Test
                        </button>
                        <button onClick={() => deleteActionMut.mutate(action.id)} className="text-red-400 hover:text-red-600"><Trash2 size={12} /></button>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          {!node.options?.length && <p className="text-center py-4 text-gray-400 text-xs">Chưa có option</p>}
        </div>
      </div>
